...
def md_14(z):
    ...
    return (0.015*(1+z)**2.7/(1 + ((1+z)/(1+1.9))**5.6) * u.Msun * u.Mpc**(-3) * u.yr**(-1)).to(u.Msun * u.Gpc**(-3) * u.yr**(-1)).value
...
def mf_17(z):
    ...
    sfr : `float or numpy.array`
        star formation rate [Msun/(Gpc^3 yr)]
    """

    sfr = 0.01 * (1 + z) ** (2.6) / (1 + ((1 + z) / 3.2) ** 6.2) * u.Msun/(u.Mpc**3 * u.yr)

    return sfr.to(u.Msun/(u.Gpc**3 * u.yr)).value
